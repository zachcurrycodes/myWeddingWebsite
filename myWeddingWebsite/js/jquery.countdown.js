...
/* Display a countdown timer.
   ...
   $('div selector').countdown(
       {until: new Date(2019, 1 - 2, 1, 0, 0, 0), onExpiry: happyNewYear}); */
	...
	$.extend(Countdown.prototype, {
		...
		_adjustSettings: function(target, inst) {
			...
			if (serverEntry != null) {
				...
				var serverResult = $.isFunction(inst.options.serverSync)
					? inst.options.serverSync.apply(target, [])
					: null;
				now = new Date();
				...
				while (matches) {
					switch (matches[2] || "s") {
						...
						case "o":
							...
							day = Math.min(
								day,
								plugin._getDaysInMonth(year, month)
							);
							break;
						case "y":
		...
		_calculatePeriods: function(inst, show, significant, now) {
			...
			if (show[Y] || show[O]) {
				...
				var lastNow = plugin._getDaysInMonth(
					now.getFullYear(),
					now.getMonth()
				);
				var lastUntil = plugin._getDaysInMonth(
					until.getFullYear(),
					until.getMonth()
				);
				var sameDay =
				...
				var lastDay = plugin._getDaysInMonth(
